...
export class ProductController {
  ...
  constructor() {
    ...
    this.getProductsByPlatformController =
      this.getProductsByPlatformController.bind(this);
    this.getProductsByGenreController =
      this.getProductsByGenreController.bind(this);
  }
  ...
  async getProductsWithLimitController(
    ...
    try {
      ...
      if (isNaN(limit) || limit <= 0)
        return ControllerHandler.badRequest("Invalid limit", res);

      const products = await this.productService.getProductsWithLimit(limit);

      return ControllerHandler.ok(
        `Products list with a limit of ${limit}`,
        res,
        products
      );
    } catch (error) {
  ...
  async getProductByIdController(
    ...
    try {
      ...
      if (isNaN(productId) || productId <= 0)
        return ControllerHandler.badRequest(`Invalid product ID: ${id}`, res);
    ...
    try {
      ...
      if (!productData.title || typeof productData.title !== "string")
        return ControllerHandler.badRequest(
          "Title most be a string and not empty",
          res
        );
      if (
      ...
      return ControllerHandler.ok(
        `Product created with ID: ${newProduct.id_product}`,
        res,
        newProduct
      );
    } catch (error) {
  ...
  async updateProductController(
    ...
    try {
      ...
      if (!updatedProduct)
        return ControllerHandler.notFound("Product not found", res);

      return ControllerHandler.ok(
        `Product updated with ID: ${id}`,
        res,
        updatedProduct
      );
    } catch (error) {
  ...
  async getProductsByPlatformController(
    ...
    try {
      ...
      const products = await this.productService.getProductsByPlatform(
        platform
      );
      return ControllerHandler.ok(
